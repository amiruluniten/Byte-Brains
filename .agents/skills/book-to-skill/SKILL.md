---
name: book-to-skill
description: "use this when you need to convert a book or document into a structured agent skill"
---
...
Books contain crystallized expertise: frameworks, principles, and techniques that took years to develop. This skill extracts that knowledge into a format the agent can leverage repeatedly.
...
- **This skill's runtime script** lives in the same folder as this SKILL.md:
  `<workspace>/.opencode/skills/book-to-skill/scripts/extract.py`
  (locate by globbing `**/book-to-skill/scripts/extract.py` if outside the workspace).
- **Generated skills go to** `SKILLS_HOME` = `<workspace>/.opencode/skills/`.
  If more than one candidate skill root exists, ask the user once and remember the answer for the session — do not silently default.
...
**Output:** Complete skill with SKILL.md, chapters/, glossary, patterns, cheatsheet, in `SKILLS_HOME`
...
**Trigger:** User provides one or more new source paths and indicates they want to update an existing skill (by pointing to the existing skill folder or requesting an update)
**Action:** Run Step 0 (out-of-scope check), Step 1 (validate inputs), Step 1.5 (identify book type), and Step 2 (extract new files). Then skip to the **Update / Fold-in Workflow**.
...
- If the last argument is not a file, folder, or glob that exists or matches any files, and it looks like a skill slug (lowercase hyphens, alphanumeric), treat it as `SKILL_NAME`.
...
Find and run the extraction script (`scripts/extract.py` next to this SKILL.md):
...
The run prints all three paths on completion (`Workdir ->`, `Text ->`, `Meta ->`). **Take the paths from that output (or from `metadata.json`'s own `workdir` field) rather than assuming a fixed location** — the directory name differs per run so concurrent extractions cannot overwrite each other.

Read that run's `metadata.json` to inspect the results.

**Always confirm the extraction is the document you asked for** before generating anything: check `filename` / `source_file` in `metadata.json`, or the `SOURCE:` header on the first line of `full_text.txt`.

---

## Step 2.5 — Pre-flight cost estimate

Read this run's `metadata.json` and present the user with an estimate **before doing any generation**:
...
- Output tokens ≈ chapters × per-chapter budget + 4,000 (SKILL.md) + 4,500 (glossary + patterns + cheatsheet)
  - Per-chapter budget midpoint by `BOOK_TYPE` (DEPTH is decided in Step 4 and can raise it): `text` ≈ 1,000, `technical` ≈ 1,800.
- Cost: report the token counts; multiply by the user's current per-1M-token input/output rates if asked. Do NOT hardcode dollar figures.
...
Treat `full_text.txt` as a queryable corpus, not a single read. Loading the whole file into context burns budget you need later for generation.
...
Under 50k tokens, a single `Read` is fine. A 200-page book is ~75k tokens; re-reading per chapter costs ~2M input tokens, while grep + sed keeps generation cost proportional to the output, not the source.
...
Set `SKILLS_HOME` (see **Locations**) and check if `$SKILLS_HOME/<skill_name>/` already exists.
...
If the user selects **Update / Fold-in**, proceed immediately to the **Update / Fold-in Workflow** (skipping Steps 3, 4, 6, 7, 8, 9).
...
**TOKEN BUDGET RULE — adaptive.** The per-chapter budget scales with `BOOK_TYPE` and `DEPTH`:
...
- **Reproduce one worked example or artifact** from the chapter under a `## Worked Example` section. This is the single biggest lever and the main thing a learner returns for.
...
If a chapter genuinely has no worked example and resists expansion, let it land below the study floor rather than padding — and note that the chapter is thin in its Core Idea. A `reference`-depth chapter deliberately omits worked examples and keeps only decision-ready essentials.
...
**This is the most differentiated layer of the skill — treat it as a reasoning aid, not a keyword list.** It captures the author's *judgment*: the decisions they'd make and why. It's the file that turns "I know the words" into "I'd act the way the author would".
...
4. **Thresholds & defaults** — the specific numbers, ratios, or rules of thumb the author commits to.
...
- Format mostly as compact tables and decision rules; content you'd want on a single printed page.
- Max 1,200 tokens.
...
**KEEP SKILL.md BODY UNDER 4,000 TOKENS.** Compaction truncates from the END — put the most important content FIRST.
...
Before reporting success or loading the skill in another session, run the advisory security scan:

```bash
"$PYTHON_BIN" "$SKILL_DIR/tools/scan_generated_skill.py" "$SKILLS_HOME/<skill_name>"
```

(`$SKILL_DIR` is this skill's folder — where `scripts/extract.py` lives.)
...
Remove **the work directory this run actually used** — the `Workdir ->` path from the extraction output, also stored as `workdir` in `metadata.json`. Never delete a directory you did not create.
...
- Read `glossary.md`, `patterns.md`, and `cheatsheet.md` to see what terms and frameworks are already indexed.

### 2. Match Content & Identify Revisions vs. Additions
Analyze the new extracted text in this run's `full_text.txt` (the `Text ->` path) to identify if the new content represents:
- **Updates/Revisions to existing chapters**: merge the new details into the existing chapter file and rewrite it.
- **New additions**: create **new chapter summary files** under `chapters/`, numbering after the highest existing chapter (e.g. after `ch12`, create `ch13-*.md`, `ch14-*.md`, etc.).
...
- Follow the formatting guidelines in **Step 7**.
- Write/update the file in `$SKILLS_HOME/<skill_name>/chapters/`.

### 4. Merge Supporting Files
- **Merge glossary.md**: combine and alphabetize existing + new terms; if a term exists, append new references (e.g. `**Term** — definition (Ch 4, Ch 13)`). Rewrite fully alphabetized.
- **Merge patterns.md**: append new techniques/algorithms/patterns, consistent formatting, keep total under 2,500 tokens.
- **Merge cheatsheet.md**: extract new comparison rules, decision tables, parameter guides; integrate cleanly.

### 5. Re-generate the Master SKILL.md
Update `$SKILLS_HOME/<skill_name>/SKILL.md`:
- **Metadata**: increment chapter count, update page count, add new source names, update `Generated` date.
- **Core Frameworks**: fold in the most high-impact mental models (keep the file under 4,000 tokens).
- **Chapter Index**: append new chapters.
- **Topic Index**: merge new topics alphabetically; append new chapter links to existing topics.

### 6. Scan, Cleanup, and Report
Run **Step 9.5**, then **Step 10** to clean up and print an update report (new chapters, merged glossary terms, updated indices).
