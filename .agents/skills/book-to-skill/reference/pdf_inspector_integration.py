...
def install_pdf_inspector_hook(utils_module: Any | None = None) -> None:
    ...
    if utils_module is None:
        from reference import utils as utils_module

    original = utils_module.extract_single_file
    if getattr(original, "_reference_pdf_inspector_hook", False):
        return
    ...
    wrapped._reference_pdf_inspector_hook = True  # type: ignore[attr-defined]
    wrapped._reference_original = original  # type: ignore[attr-defined]
...
def enrich_pdf_inspector_metadata(metadata_path: str | Path | None = None) -> None:
    ...
    if metadata_path is None:
        from reference.config import OUTPUT_META
