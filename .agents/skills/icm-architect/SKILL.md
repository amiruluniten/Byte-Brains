---
name: icm-architect
description: Design any process, idea, problem, or body of knowledge into an ICM (Interpretable Context Methodology) workspace — folder structure as agent architecture — or restructure an existing folder, repo, or vault into one. Use when the user wants to (1) turn a recurring workflow into an agent-runnable folder pipeline, (2) organize scattered notes, files, or knowledge into a library one AI agent can walk, (3) map a team or company as connected context ("second brain", "team brain"), (4) "map this repo" or "what would a change hit" — audit a codebase or mixed folder into a walkable edit map, (5) restructure an existing workspace to ICM conventions, or (6) says "make this an ICM", "ICM this", "audit this folder", "build me a workspace", or "structure this for agents".
---
...
1. **One folder, one job** (`core.md`: one stage, one job). Each folder does a single step or holds a single kind of thing, and states its own purpose in a file inside itself. The structure is the documentation.
...
5. **Factory vs. product** (`core.md`: configure the factory, not the product). Reference material (rules, voice, schemas, templates — stable across runs) lives structurally apart from working artifacts (outputs, drafts — new every run). Configure the factory once; the product is what each run emits.
6. **Every output is an edit surface** (`core.md`: principle 4). Intermediate outputs are plain files a human can open, edit, and save before the next step reads them. Nothing moves forward until a person has read the last output.
7. **Load only what the step needs** (`core.md`: layered context loading). An agent executing a step reads its contract, its references, and its inputs — not the whole workspace. 2,000–8,000 tokens per step is the healthy range.
...
- **A body of work later agents must edit** (code, markdown, or mixed) → System map form. Read [references/system-map.md](references/system-map.md) after picking the form. In a repo that also uses the `codebase-design` vocabulary, the system map records *what a change hits* (objects, movements, impact); deepening opportunities — modules to deepen, seams to move — are not a map deliverable. Note them and hand them to `improve-codebase-architecture`, which enters the main flow via `grill-with-docs`.
...
If the host has the `grilling` skill, run this dialogue through it: map the questions as a design tree, work in numbered rounds on the frontier, look up facts in the environment yourself, and put every decision to the user. The questions above are the content; `grilling` is the format. ICM's addition to that interview is the extraction step: mine the answers for stages, human gates, and factory/product splits, then write them into the workspace's contracts. Done when the user can confirm the structure back in one pass — repeating unit, stages, human gates, factory vs product — with no open questions; under `grilling`, that is the frontier-empty condition.
...
**4. Write the contracts.** Root `CLAUDE.md` (identity + routing table), root `CONTEXT.md` (the pipeline or schema definition), one `CONTEXT.md` per stage/hub folder, `setup/questionnaire.md` if the factory needs configuring per user. Write inputs as explicit file paths, split into working (this run) and reference (every run).
   ...
   **Sharing a repo with the domain-modeling flow?** In a repo whose root `CONTEXT.md` is a domain glossary (the `domain-modeling` / `grill-with-docs` convention) or which has a root `CONTEXT-MAP.md` marking bounded contexts, those names are taken: name stage contracts `STEP.md` instead (content unchanged), and treat the existing glossary as factory reference material — link to it, never copy or extend it with process details. Dedicated ICM workspaces (workspace root ≠ code repo root) keep the standard names.
...
- **Restructure on its own session, never mid-build.** Do not run Restructure mode while a main flow (`grill-with-docs` → `/to-spec` → `/to-tickets` → `/implement`) has work in flight in the same repo: file moves churn the diff that `/implement` and `/code-review` are working against. Sequence them — restructure between features, and commit the restructure before the next feature's first red-green slice. Same for System map audits: map between features, not during them.
