...
export interface DecompositionSpec {
  ...
  /** Cumulative real gap (RM million) over `gapYears`, from the fragment. */
  cumulativeGapRmMillion: number;
  gapYears: { from: number; to: number };
  basisNotes: string[];
  stagnation: {
...
export function buildDecomposition(frag: MissingBillionsFragment): DecompositionSpec {
  ...
  const from = frag.anchor_year + 1;
  return {
    anchorYear: frag.anchor_year,
    indexed,
    perVisitor,
    cumulativeGapRmMillion: cumulativeMissingBillions(frag, from, latest.year),
    gapYears: { from, to: latest.year },
    basisNotes: [
    ...
    stagnation: {
      ...
      latestPerVisitorRealRm: latest.per_visitor_real_2019_rm,
    },
  };
}
